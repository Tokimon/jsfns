...
describe('"append"', () => {
	...
	describe('Typing', () => {
		...
		it('Defaults to `HTMLDivElement` for markup it cannot reflect in the type (tagged selector or HTML with attributes)', () => {
			// The tag is still parsed correctly at runtime (see "With an explicit root" above) -
			// this default only affects the static type, since safely parsing an arbitrary tag
			// name out of a template literal type is not practical (see `append<E>` below for an override)
			expectTypeOf(append('span.my-class')).toEqualTypeOf<HTMLDivElement | null>();
			expectTypeOf(append('<input type="text">')).toEqualTypeOf<HTMLDivElement | null>();
		});

		it('Falls back to `HTMLDivElement` for a non-literal (widened) string', () => {
			const selector: string = '.my-class';
			expectTypeOf(append(selector)).toEqualTypeOf<HTMLDivElement | null>();
			expectTypeOf(append(testNode, selector)).toEqualTypeOf<HTMLDivElement | null>();
		});

		it('Allows an explicit type argument to override the inferred/defaulted type', () => {
			expectTypeOf(
				append<HTMLInputElement>('<input type="text">'),
			).toEqualTypeOf<HTMLInputElement | null>();
			expectTypeOf(
				append<HTMLInputElement>(testNode, '<input type="text">'),
			).toEqualTypeOf<HTMLInputElement | null>();
		});
	});
});
