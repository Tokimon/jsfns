...
describe('"vendorPrefixed"', () => {
	it('Prefixes a word with vendor prefixes', () => {
		expect(vendorPrefixed('vendorPrefixed')).toEqual(
			resultArray('VendorPrefixed', 'vendor-prefixed'),
		);
	});

	it('Prefixes a phrase with vendor prefixes and converts it into PascalCased word', () => {
		expect(vendorPrefixed('prefix this phrase')).toEqual(
			resultArray('PrefixThisPhrase', 'prefix-this-phrase'),
		);
	});
});
